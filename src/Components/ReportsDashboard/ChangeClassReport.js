...
import InstructorChangeAccordionScale from '../AccordionScale/InstructorChangeAccordionScale';
              ...
              <div style={{ margin: '20px 0px' }}>
                {strengthsArr[0].Strengths.length === 0 ? <li>[empty]</li> : (
                  strengthsArr[0].Strengths.map((scale, index) => (
                    <InstructorChangeAccordionScale scale={scale} index={index} scoreType={props.scoreType} scoreCohort={props.scoreCohort} level={'high'} />
                  ))
                )}
              </div>
              ...
              <div style={{ margin: '20px 0px' }}>
                {strengthsArr[1].Developing_Strengths.length === 0 ? <li>[empty]</li> : (
                  strengthsArr[1].Developing_Strengths.map((scale, index) => (
                    <InstructorChangeAccordionScale scale={scale} index={index} scoreType={props.scoreType} scoreCohort={props.scoreCohort} level={'moderate'} />
                  ))
                )}
              </div>
              ...
              <div style={{ margin: '20px 0px' }}>
                {strengthsArr[2].Growth_Areas.length === 0 ? <li>[empty]</li> : (
                  strengthsArr[2].Growth_Areas.map((scale, index) => (
                    <InstructorChangeAccordionScale scale={scale} index={index} scoreType={props.scoreType} scoreCohort={props.scoreCohort} level={'low'} />
                  ))
                )}
              </div>
            </Card.Body>
